  ...
  const ProductCard = ({ id, category, name, originalPrice, currentPrice, image }) => (
    ...
    >
      <img
        ...
        alt={name}
        loading="lazy"
        className="w-full h-56 object-cover bg-amber-50 transition-transform duration-500 hover:scale-105"
        itemProp="image"
      />
      <div className="p-4">
        <p className="text-[11px] uppercase tracking-wide text-amber-700 font-medium mb-1" itemProp="category">
          {category}
        </p>
        <h3 className="text-base font-semibold text-gray-900 mb-2 line-clamp-2" itemProp="name">
          {name}
        </h3>
        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs text-gray-400 line-through" itemProp="price">₹{originalPrice}</span>
          <span className="text-lg font-bold text-amber-600" itemProp="price">₹{currentPrice}</span>
        </div>
  ...
  return (
    <section className="bg-gray-50 min-h-screen" id="silver-collection">
      <div className="max-w-7xl mx-auto px-4 lg:pb-10">
        {/* Tabs */}
        <div className="flex overflow-x-auto sm:justify-center gap-3 sm:gap-8 mb-8 border-b border-gray-200 pb-3 no-scrollbar">
          {tabs.map((tab) => (
            <button
              key={tab.value}
              onClick={() => setActiveTab(tab.value)}
              className={`px-4 sm:px-6 py-2 font-semibold text-sm sm:text-base uppercase relative transition-all whitespace-nowrap ${
                activeTab === tab.value
                  ? 'text-amber-800'
